from utils.utils import term_checker, get_path, answer_handler
from query.unify import is_variable
...
from query.pq import SearchQueue
from utils.utils import fact_binary_search, child_to_parent, parent_inherits, child_assigned
from utils.numeric import prob_calc, filter_eq
...
def rule_query(kb, expr, cut, show_path):
    ...
    while not queue.empty: ## keep searching until it is empty meaning nothing left to be searched
        ...
        if current_goal.ind >= len(current_goal.fact.rhs): ## all rule goals have been searched
            if current_goal.parent is None: ## no more parents 
            ...
            if show_path:
                path.append(current_goal.domain)
            continue
